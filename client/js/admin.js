        ...
        if (qrPlaceholder) {
          ...
          if (data.status === 'ready') {
            qrPlaceholder.innerHTML = `
              <i class="fa-solid fa-satellite-dish" style="font-size: 3.5rem; color: #10b981; margin-bottom: 0.75rem; display: block;"></i>
              <span style="font-weight: 800; color: var(--text-main); font-size: 1rem;">WhatsApp Gateway Active</span>
              <p style="color: var(--text-muted); font-size: 0.8rem; margin: 0.35rem 0 0.85rem 0; line-height: 1.4;">Automated civic notification gateway is active. Dispatch test messages on the right.</p>
              <div style="background: rgba(13, 148, 136, 0.08); border: 1px dashed var(--primary); padding: 0.5rem; border-radius: 8px; font-size: 0.72rem; color: var(--text-main);">
                <i class="fa-solid fa-circle-check" style="color: #10b981; margin-right: 0.25rem;"></i> System Notifications & Direct Dispatch Enabled
              </div>
            `;
          } else if (data.status === 'connecting') {
            qrPlaceholder.innerHTML = `
